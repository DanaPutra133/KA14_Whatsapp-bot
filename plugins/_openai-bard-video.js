...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  try {
    ...
    try {
      let buffer = await q.download();
      media = await uploadFile(buffer);
      urlAPI = `https://api.botcahx.eu.org/api/search/bard-video?url=${media}&text=${text}&apikey=${btc}`;
...
handler.help = ['bardvideo'];
handler.tags = ['tools'];
handler.command = /^(bardvideo)$/i;
handler.limit = true;
