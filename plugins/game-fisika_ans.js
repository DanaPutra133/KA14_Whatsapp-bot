...
handler.before = async function (m) {
    ...
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*fska/i.test(m.quoted.text)) return !0
    this.fisika = this.fisika ? this.fisika : {}
    if (!(id in this.fisika)) return m.reply('Soal itu telah berakhir')
    if (m.quoted.id == this.fisika[id][0].id) {
        let json = JSON.parse(JSON.stringify(this.fisika[id][1]))
